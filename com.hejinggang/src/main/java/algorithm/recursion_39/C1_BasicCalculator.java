...
public class C1_BasicCalculator {
    ...
    private static void push(ArrayList<Integer> numbers, ArrayList<Character> ops, int cur, Character op) {
        int n = numbers.size();
    ...
    private static int compute(ArrayList<Integer> numbers, ArrayList<Character> ops) {
        ...
        for (int i = 1; i < n; i++) {
            ans = ops.get(i - 1) == '+' ? ans + numbers.get(i) : ans - numbers.get(i);
        }
        return ans;
    }

    public static void main(String[] args) {
        String str = "30+6*2-((5+3)*2+1)";
        System.out.println(calculate(str));
        int ans = 30 + 6 * 2 - ((5 + 3) * 2 + 1);
